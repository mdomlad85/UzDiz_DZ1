...
namespace ToF.Builder
{
    ...
    /// </summary>
    public class NasumicniTofSustavBuilder : ITofSustavBuilder
    {
        ...
        public void UcitajPostavke(object ulazniPodaci)
        {
            if (ulazniPodaci is string[] && IsIspravnePostavke((string[])ulazniPodaci))
            {
                ...
                _tofSustav.Postavke.AlgoritamProvjere = DohvatiAlgoritamProvjereNasumicno();
        ...
        private ITesterUredjaja DohvatiAlgoritamProvjereNasumicno()
        {
            var max = Enum.GetNames(typeof(TipTestera)).Length + 1;
            var tip = (TipTestera)(_tofSustav.GeneratorBrojeva.Next(max));
